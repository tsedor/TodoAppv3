...
class TodoList extends React.Component {
  componentDidMount() {
    this.props.logged && this.props.fetchTodos();
  }
  render() {
...
const searchSelector = (todos, searchValue) => todos.filter(todo => todo.text.toLowerCase().includes(searchValue.toLowerCase()))
const sortAsc = todos => todos.sort((t1, t2) => t1.text < t2.text ? -1 : 1);
const sortDesc = todos => todos.sort((t1, t2) => t1.text > t2.text ? -1 : 1);
const sorting = (todos, sorting) => sorting === 'asc' ? sortAsc(todos) : sortDesc(todos);

const filter = (todos, active, completed) => todos.filter(todo => todo.done === false & active === true || todo.done === true & completed === true);

const mapStateToProps = state => ({
  logged: state.user.logged,
  todos: filter(sorting(searchSelector(state.todos.todos, state.todos.searchValue), state.todos.sorting), state.todos.filter.active, state.todos.filter.completed)
})
